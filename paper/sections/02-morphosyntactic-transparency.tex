...
The pilot reported with the joint analysis supplies a bounded check on the category contrast. In a sample of 50 \mention{call} hits, 17 were genuine AP-nucleus TFRs and 14 occurred in the externally predicative position. Checks of every \mention{seem} or \mention{appear} return in the query set found no genuine non-NP nucleus; the per-query totals are being re-verified against COCA, and the working paper carries the details. This is distributional evidence, not a completed acceptability study; the relevant category-discriminating environments and judgments remain to be added to the joint paper.
