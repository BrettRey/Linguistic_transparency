...
The agreement prediction couldn't be tested. COCA has 91 tokens of \mention{piles of money} and one followed by a finite form of \mention{be}, and the discriminating cell across all non-count complements holds two tokens, split one to one. That is a limitation worth stating in general terms: the override prediction is corpus-testable only for members that appear as subjects, and quantificational \mention{piles} is overwhelmingly an object (\mention{he made piles of money}). The same sparsity blocked the inanimate \mention{bunch} contrast in §\ref{sec:2-2}. Candidate-member discovery may therefore need acceptability judgments rather than frequency, which is a constraint on how the profile can be tested rather than evidence about the profile. Protocol, predictions fixed in advance, and full returns in \texttt{paper/data/piles-heldout-test.md}.
