...
So the override prediction, which is the profile's headline, is corpus-testable only for members that appear in subject position, and candidate-member discovery for this class will need acceptability judgments rather than frequency. That is a constraint on how the profile can be tested rather than evidence about the profile, and it is worth more to state than a confirmation would have been. Protocol with predictions fixed in advance, full returns, and one correction it forced on Appendix~\ref{a.2-partitive-vs.-pseudo-partitive-of-complements} are in \texttt{paper/data/piles-heldout-test.md}.
