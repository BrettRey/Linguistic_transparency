...
The oblique contrasts in (\ref{ex:partitive-plural-oblique}) are highly structured, though not in the syntactic way they first appear to be. (\ref{ex:partitive-plural-oblique}b) carries no determiner and is common (\mention{the rest of society} alone has 442 COCA tokens). What \mention{rest} and \mention{remainder} require is a complement that already denotes uniquely, which bare generics and institution nouns do unaided; what they resist is a bare complement introducing a novel indefinite subset. The condition is semantic, not a demand for an overt determiner.
...
\mention{remainder} & \mention{the} & $\checkmark$ (\mention{the remainder of the time}) & $\checkmark$ if uniquely denoting (\mention{the remainder of life}) & sg/pl (complement-controlled) \\
\mention{rest} & \mention{the} & $\checkmark$ (\mention{the rest of the time}) & $\checkmark$ if uniquely denoting (\mention{the rest of society}) & sg/pl (complement-controlled) \\
\addlinespace
...
The agreement prediction couldn't be tested at all. COCA has 91 tokens of \mention{piles of money} and one followed by a finite form of \mention{be}; adding COHA's two centuries brings the discriminating non-count cell to three tokens, one singular and two plural. Nothing there supports the override and three tokens cannot weigh against it either. A container-noun control (\mention{sacks/boxes/crates of money}) returns nothing at all, which locates the problem: quantity and container expressions of this shape occur as objects, not as subjects of finite \mention{be}. The same sparsity blocked the inanimate \mention{bunch} contrast in §\ref{sec:2-2}.
