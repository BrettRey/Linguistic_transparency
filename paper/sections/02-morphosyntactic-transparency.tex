...
The contrast in (\ref{ex:plural-only-of-complements}) shows the plural-only pattern: the first is the characteristic use, the second a marked one that the corpus finds only rarely (\mention{lots of the people} and \mention{lots of the time} are attested, so the frame isn't blocked). \mention{Remainder} and \mention{rest} take only partitive \mention{of}-complements in their characteristic use; \mention{number} and \mention{couple}₂ take plural obliques.
...
\mention{lots, bags, heaps, loads, oodles, stacks} & usually without a determiner & $\checkmark$, strongly dispreferred (COCA pilot: $\approx\!0.2\%$, §\ref{sec:2-1}) & $\checkmark$ & pl → sg with non-count complement \\
\addlinespace
