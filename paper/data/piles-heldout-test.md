...
**Follow-up (2026-07-26): the same sweep run across the whole subgroup.** `lots/heaps/loads/bags/oodles/stacks of the *` plus `plenty of the *` return 465 tokens for the plural-only subgroup (374 distinct complements) and 169 for *plenty* (140), against 13 and 2 from the hand-picked complements. Two consequences for this file. First, **P4's comparator is superseded**: the "≈0.2%" cited in the prediction table came from the narrow probe and no defensible overall share is now computable, since the two frames were sampled over different complement inventories. P4's *verdict* stands unchanged on its stated criterion, `piles of the N` being a small fraction of `piles of N`; only the parenthetical benchmark is stale. The prediction text is left as pre-registered rather than rewritten. Second, and more interesting, **the sense-discrimination finding generalises**: the determined frame recruits the literal homonym for every subgroup member that has one (*stacks* → library stacks and smokestacks, *heaps* → literal and colliery heaps, *bags*/*loads* → containers and cargo, *lots* → pharmaceutical batches), while *oodles*, the only member with no literal homonym, returns quantificational hits exclusively. What looked like an accident of this candidate member is a subgroup regularity, and it yields a new prediction: a candidate with a literal homonym should show the split, one without shouldn't. §2.5 now reports this.
